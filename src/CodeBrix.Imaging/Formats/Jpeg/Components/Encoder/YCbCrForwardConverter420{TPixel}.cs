    ...
    where TPixel : unmanaged, IPixel<TPixel>
{
    ...
    /// </summary>
    private IRgbToYCbCrConverterLut colorTables;
    ...
    public YCbCrForwardConverter420(ImageFrame<TPixel> frame)
    {
        ...
        if (ArchitectureInfo.IsRiscVArchitecture) //Use "safe" version of RgbToYCbCrConverterLut for RISC-V - this might not be necessary with newer versions of .NET; or RVA23 boards
        {
            this.colorTables = RgbToYCbCrConverterLutSafe.Create();
        }
        else if (!RgbToYCbCrConverterVectorized.IsSupported)
        {
            this.colorTables = RgbToYCbCrConverterLutUnsafe.Create();
        }
        else
        {
            this.colorTables = default(RgbToYCbCrConverterLutUnsafe);
        }
    }
