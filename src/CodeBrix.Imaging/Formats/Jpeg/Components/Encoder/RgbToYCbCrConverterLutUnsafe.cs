...
internal interface IRgbToYCbCrConverterLut
{
    void Convert444(Span<Rgb24> rgbSpan, ref Block8x8F yBlock, ref Block8x8F cbBlock, ref Block8x8F crBlock);
    void Convert420(Span<Rgb24> rgbSpan, ref Block8x8F yBlockLeft, ref Block8x8F yBlockRight, ref Block8x8F cbBlock,
        ref Block8x8F crBlock, int row);
}

/// <summary>
/// Provides 8-bit lookup tables for converting from Rgb to YCbCr color space.
/// Methods to build the tables are based on libjpeg implementation.
/// </summary>
internal unsafe struct RgbToYCbCrConverterLutUnsafe : IRgbToYCbCrConverterLut
{
    /// <summary>
    ...
    /// </summary>
    /// <returns>The initialized <see cref="IRgbToYCbCrConverterLut"/> instance</returns>
    public static IRgbToYCbCrConverterLut Create()
    {
        RgbToYCbCrConverterLutUnsafe tables = default;
    ...
    private float CalculateY(byte r, byte g, byte b)
    {
        ...
        return (YRTable[r] + YGTable[g] + YBTable[b]) >> ScaleBits;
    }
    ...
    private float CalculateCb(byte r, byte g, byte b)
    {
        ...
        return (CbRTable[r] + CbGTable[g] + CbBTable[b]) >> ScaleBits;
    }
    ...
    private float CalculateCr(byte r, byte g, byte b)
    {
        ...
        return (CbBTable[r] + CrGTable[g] + CrBTable[b]) >> ScaleBits;
    }
    ...
    public void Convert444(Span<Rgb24> rgbSpan, ref Block8x8F yBlock, ref Block8x8F cbBlock, ref Block8x8F crBlock)
    {
        ...
        {
            var c = Unsafe.Add(ref rgbStart, i);

            yBlock[i] = CalculateY(c.R, c.G, c.B);
            cbBlock[i] = CalculateCb(c.R, c.G, c.B);
            crBlock[i] = CalculateCr(c.R, c.G, c.B);
        }
    }
    ...
    public void Convert420(Span<Rgb24> rgbSpan, ref Block8x8F yBlockLeft, ref Block8x8F yBlockRight, ref Block8x8F cbBlock, ref Block8x8F crBlock, int row)
    {
        ...
        ref var yBlockLeftRef = ref Unsafe.As<Block8x8F, float>(ref yBlockLeft);
        ref var yBlockRightRef = ref Unsafe.As<Block8x8F, float>(ref yBlockRight);

        // 0-31 or 32-63
        // upper or lower part
        var chromaWriteOffset = row * (Block8x8F.Size / 2);
        ref var cbBlockRef = ref Unsafe.Add(ref Unsafe.As<Block8x8F, float>(ref cbBlock), chromaWriteOffset);
        ref var crBlockRef = ref Unsafe.Add(ref Unsafe.As<Block8x8F, float>(ref crBlock), chromaWriteOffset);

        ref var rgbStart = ref rgbSpan[0];

        for (var i = 0; i < 8; i += 2)
        {
            var yBlockWriteOffset = i * 8;
            ref var stride = ref Unsafe.Add(ref rgbStart, i * 16);
            var chromaOffset = 8 * (i / 2);
    ...
    private void ConvertChunk420(ref Rgb24 stride, ref float yBlock, ref float cbBlock, ref float crBlock)
    {
        ...
        for (var k = 0; k < 8; k += 2)
        {
            ref var yBlockRef = ref Unsafe.Add(ref yBlock, k);

            // top row
            var px0 = Unsafe.Add(ref stride, k);
            var px1 = Unsafe.Add(ref stride, k + 1);
            yBlockRef = CalculateY(px0.R, px0.G, px0.B);
            Unsafe.Add(ref yBlockRef, 1) = CalculateY(px1.R, px1.G, px1.B);

            // bottom row
            var px2 = Unsafe.Add(ref stride, k + 16);
            var px3 = Unsafe.Add(ref stride, k + 17);
            Unsafe.Add(ref yBlockRef, 8) = CalculateY(px2.R, px2.G, px2.B);
            Unsafe.Add(ref yBlockRef, 9) = CalculateY(px3.R, px3.G, px3.B);

            // chroma average for 2x2 pixel block
            Unsafe.Add(ref cbBlock, k / 2) = CalculateAverageCb(px0, px1, px2, px3);
            Unsafe.Add(ref crBlock, k / 2) = CalculateAverageCr(px0, px1, px2, px3);
        }
    }
    ...
    {
        return 0.25f
               * (CalculateCb(px0.R, px0.G, px0.B)
                  + CalculateCb(px1.R, px1.G, px1.B)
                  + CalculateCb(px2.R, px2.G, px2.B)
                  + CalculateCb(px3.R, px3.G, px3.B));
    }
    ...
    {
        return 0.25f
               * (CalculateCr(px0.R, px0.G, px0.B)
                  + CalculateCr(px1.R, px1.G, px1.B)
                  + CalculateCr(px2.R, px2.G, px2.B)
                  + CalculateCr(px3.R, px3.G, px3.B));
    }
