...
internal static class LossyUtils
{
    ...
    public static int Vp8_SseNxN(Span<byte> a, Span<byte> b, int w, int h)
    {
        ...
        for (var y = 0; y < h; y++)
        {
            for (var x = 0; x < w; x++)
            {
                var diff = a[aOffset + x] - b[bOffset + x];
                count += diff * diff;
            }
    ...
    {
        var d = 0;
        var dataSize = (4 * WebpConstants.Bps) - 16;
        for (var y = 0; y < 16 * WebpConstants.Bps; y += 4 * WebpConstants.Bps)
        {
...
#endif
        {
            var sum1 = TTransform(a, w, scratch);
            var sum2 = TTransform(b, w, scratch);
    ...
    {
        var offsetMinus1 = offset - 1;
        var offsetMinusBps = offset - WebpConstants.Bps;
        var dc = 16;
        for (var j = 0; j < 16; j++)
        {
    ...
    {
        // vertical
        var src = yuv.Slice(offset - WebpConstants.Bps, 16);
        for (var j = 0; j < 16; j++)
        {
    ...
    {
        // vertical
        var src = yuv.Slice(offset - WebpConstants.Bps, 8);

        var endIdx = 8 * WebpConstants.Bps;
        for (var j = 0; j < endIdx; j += WebpConstants.Bps)
        {
    ...
    public static void DC8uvNoTop(Span<byte> dst, Span<byte> yuv, int offset)
    {
        ...
        var endIdx = 8 * WebpConstants.Bps;
        for (var i = 0; i < endIdx; i += WebpConstants.Bps)
        {
            // dc0 += dst[-1 + i * BPS];
    ...
    public static void DC4(Span<byte> dst, Span<byte> yuv, int offset)
    {
        ...
        dc >>= 3;
        var endIndx = 4 * WebpConstants.Bps;
        for (var i = 0; i < endIndx; i += WebpConstants.Bps)
        {
    ...
    {
        // vertical
        var topOffset = offset - WebpConstants.Bps;
        ...
        var endIdx = 4 * WebpConstants.Bps;
        for (var i = 0; i < endIdx; i += WebpConstants.Bps)
        {
    ...
    public static void HE4(Span<byte> dst, Span<byte> yuv, int offset)
    {
        // horizontal
        var offsetMinusOne = offset - 1;
        var a = yuv[offsetMinusOne - WebpConstants.Bps];
        var b = yuv[offsetMinusOne];
        var c = yuv[offsetMinusOne + WebpConstants.Bps];
        var d = yuv[offsetMinusOne + (2 * WebpConstants.Bps)];
        var e = yuv[offsetMinusOne + (3 * WebpConstants.Bps)];
        var val = 0x01010101U * Avg3(a, b, c);
    ...
    public static void RD4(Span<byte> dst, Span<byte> yuv, int offset)
    {
        // Down-right
        var offsetMinusOne = offset - 1;
        var i = yuv[offsetMinusOne];
        var j = yuv[offsetMinusOne + (1 * WebpConstants.Bps)];
        var k = yuv[offsetMinusOne + (2 * WebpConstants.Bps)];
        var l = yuv[offsetMinusOne + (3 * WebpConstants.Bps)];
        var x = yuv[offsetMinusOne - WebpConstants.Bps];
        var a = yuv[offset - WebpConstants.Bps];
        var b = yuv[offset + 1 - WebpConstants.Bps];
        var c = yuv[offset + 2 - WebpConstants.Bps];
        var d = yuv[offset + 3 - WebpConstants.Bps];
    ...
    public static void LD4(Span<byte> dst, Span<byte> yuv, int offset)
    {
        ...
        var c = yuv[offset + 2 - WebpConstants.Bps];
        var d = yuv[offset + 3 - WebpConstants.Bps];
        var e = yuv[offset + 4 - WebpConstants.Bps];
        var f = yuv[offset + 5 - WebpConstants.Bps];
        var g = yuv[offset + 6 - WebpConstants.Bps];
        var h = yuv[offset + 7 - WebpConstants.Bps];
    ...
    public static void HD4(Span<byte> dst, Span<byte> yuv, int offset)
    {
        // Horizontal-Down
        var i = yuv[offset - 1];
        var j = yuv[offset - 1 + (1 * WebpConstants.Bps)];
        var k = yuv[offset - 1 + (2 * WebpConstants.Bps)];
        var l = yuv[offset - 1 + (3 * WebpConstants.Bps)];
        var x = yuv[offset - 1 - WebpConstants.Bps];
        var a = yuv[offset - WebpConstants.Bps];
        var b = yuv[offset + 1 - WebpConstants.Bps];
        var c = yuv[offset + 2 - WebpConstants.Bps];
    ...
    public static void TransformWht(Span<short> input, Span<short> output, Span<int> scratch)
    {
        ...
        for (var i = 0; i < 4; i++)
        {
            ...
            var iPlus8 = 8 + i;
            var iPlus12 = 12 + i;
            var a0 = input[i] + input[iPlus12];
            var a1 = input[iPlus4] + input[iPlus8];
            var a2 = input[iPlus4] - input[iPlus8];
            var a3 = input[i] - input[iPlus12];
            tmp[i] = a0 + a1;
            tmp[iPlus8] = a0 - a1;
            tmp[iPlus4] = a3 + a2;
            tmp[iPlus12] = a3 - a2;
        ...
        {
            var imul4 = i * 4;
            var dc = tmp[0 + imul4] + 3;
            var a0 = dc + tmp[3 + imul4];
            var a1 = tmp[1 + imul4] + tmp[2 + imul4];
            var a2 = tmp[1 + imul4] - tmp[2 + imul4];
            var a3 = dc - tmp[3 + imul4];
    ...
    public static int TTransform(Span<byte> input, Span<ushort> w, Span<int> scratch)
    {
        ...
        for (var i = 0; i < 4; i++)
        {
            var inputOffsetPlusOne = inputOffset + 1;
            var inputOffsetPlusTwo = inputOffset + 2;
            var inputOffsetPlusThree = inputOffset + 3;
            var a0 = input[inputOffset] + input[inputOffsetPlusTwo];
            var a1 = input[inputOffsetPlusOne] + input[inputOffsetPlusThree];
            var a2 = input[inputOffsetPlusOne] - input[inputOffsetPlusThree];
            var a3 = input[inputOffset] - input[inputOffsetPlusTwo];
            tmp[0 + (i * 4)] = a0 + a1;
            tmp[1 + (i * 4)] = a3 + a2;
            tmp[2 + (i * 4)] = a3 - a2;
            tmp[3 + (i * 4)] = a0 - a1;
    ...
    public static void TransformOne(Span<short> src, Span<byte> dst, Span<int> scratch)
    {
...
#endif
        {
            var tmp = scratch.Slice(0, 16);
            var tmpOffset = 0;
            for (var srcOffset = 0; srcOffset < 4; srcOffset++)
            {
                // vertical pass
                var srcOffsetPlus4 = srcOffset + 4;
                var srcOffsetPlus8 = srcOffset + 8;
                var srcOffsetPlus12 = srcOffset + 12;
                var a = src[srcOffset] + src[srcOffsetPlus8];
                var b = src[srcOffset] - src[srcOffsetPlus8];
                var c = Mul2(src[srcOffsetPlus4]) - Mul1(src[srcOffsetPlus12]);
                var d = Mul1(src[srcOffsetPlus4]) + Mul2(src[srcOffsetPlus12]);
                tmp[tmpOffset++] = a + d;
                tmp[tmpOffset++] = b + c;
                tmp[tmpOffset++] = b - c;
                tmp[tmpOffset++] = a - d;
            ...
            {
                // horizontal pass
                var tmpOffsetPlus4 = tmpOffset + 4;
                var tmpOffsetPlus8 = tmpOffset + 8;
                var tmpOffsetPlus12 = tmpOffset + 12;
                var dc = tmp[tmpOffset] + 4;
                var a = dc + tmp[tmpOffsetPlus8];
                var b = dc - tmp[tmpOffsetPlus8];
                var c = Mul2(tmp[tmpOffsetPlus4]) - Mul1(tmp[tmpOffsetPlus12]);
                var d = Mul1(tmp[tmpOffsetPlus4]) + Mul2(tmp[tmpOffsetPlus12]);
...
#endif
        {
            var offset4mulstride = offset + (4 * stride);
    ...
    private static void TrueMotion(Span<byte> dst, Span<byte> yuv, int offset, int size)
    {
        ...
        var topOffset = offset - WebpConstants.Bps;
        var top = yuv.Slice(topOffset);
        var p = yuv[topOffset - 1];
        var leftOffset = offset - 1;
        var left = yuv[leftOffset];
    ...
    private static void DoFilter2(Span<byte> p, int offset, int step)
    {
        ...
        var a = (3 * (q0 - p0)) + WebpLookupTables.Sclip1(p1 - q1);
    ...
    private static void DoFilter4(Span<byte> p, int offset, int step)
    {
        ...
        var offsetMinus2Step = offset - (2 * step);
    ...
    private static void DoFilter6(Span<byte> p, int offset, int step)
    {
        ...
        var step2 = 2 * step;
        var step3 = 3 * step;
        var offsetMinusStep = offset - step;
        ...
        var a1 = ((27 * a) + 63) >> 7;  // eq. to ((3 * a + 7) * 9) >> 7
        var a2 = ((18 * a) + 63) >> 7;  // eq. to ((2 * a + 7) * 9) >> 7
        var a3 = ((9 * a) + 63) >> 7;  // eq. to ((1 * a + 7) * 9) >> 7
    ...
    {
        var end = 8 * WebpConstants.Bps;
        for (var j = 0; j < end; j += WebpConstants.Bps)
        {
