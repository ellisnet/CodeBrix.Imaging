...
{
    /// <summary>
    /// The maximum allowable value for image width or height.
    /// This limit prevents integer overflow in area calculations (width * height)
    /// and guards against excessive memory allocation from unreasonable dimensions.
    /// </summary>
        ...
        : base(configuration, width, height, metadata)
    {
        ...
        Guard.MustBeLessThanOrEqualTo(width, MaxDimension, nameof(width));
        Guard.MustBeLessThanOrEqualTo(height, MaxDimension, nameof(height));
