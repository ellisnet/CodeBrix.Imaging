...
namespace CodeBrix.Imaging.Infrastructure;
...
internal static class ThrowHelper
{
    ...
    {
        ThrowArgumentOutOfRangeException(name,
            $"Parameter \"{name}\" ({typeof(T)}) must be less than {max}, was {value}");
    }

    /// <summary>
    ///     Throws an <see cref="ArgumentOutOfRangeException" /> when <see cref="Guard.MustBeLessThanOrEqualTo{TValue}" />
    ///     fails.
    /// </summary>
    [MethodImpl(MethodImplOptions.NoInlining)]
    ...
    {
        ThrowArgumentOutOfRangeException(name,
            $"Parameter \"{name}\" ({typeof(T)}) must be less than or equal to {maximum}, was {value}");
    }
    ...
    {
        ThrowArgumentOutOfRangeException(name,
            $"Parameter \"{name}\" ({typeof(T)}) must be greater than {minimum}, was {value}");
    }

    /// <summary>
    ///     Throws an <see cref="ArgumentOutOfRangeException" /> when <see cref="Guard.MustBeGreaterThanOrEqualTo{TValue}" />
    ///     fails.
    /// </summary>
    [MethodImpl(MethodImplOptions.NoInlining)]
    ...
    {
        ThrowArgumentOutOfRangeException(name,
            $"Parameter \"{name}\" ({typeof(T)}) must be greater than or equal to {minimum}, was {value}");
    }

    /// <summary>
    ///     Throws an <see cref="ArgumentOutOfRangeException" /> when <see cref="Guard.MustBeBetweenOrEqualTo{TValue}" />
    ///     fails.
    /// </summary>
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static void ThrowArgumentOutOfRangeExceptionForMustBeBetweenOrEqualTo<T>(T value, T minimum, T maximum,
        string name)
    {
        ThrowArgumentOutOfRangeException(name,
    ...
    /// <summary>
    ///     Throws an <see cref="ArgumentException" /> when
    ///     <see cref="Guard.MustBeSizedAtLeast{T}(ReadOnlySpan{T},int,string)" /> fails.
    /// </summary>
    [MethodImpl(MethodImplOptions.NoInlining)]
