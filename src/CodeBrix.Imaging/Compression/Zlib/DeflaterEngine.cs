...
// Buffer size constants for DebugGuard assertions:
// window buffer: 2 * WSIZE (65536 bytes)
// head buffer: HASH_SIZE (32768 shorts)
// prev buffer: WSIZE (32768 shorts)

namespace CodeBrix.Imaging.Compression.Zlib; //Was previously: namespace SixLabors.ImageSharp.Compression.Zlib;
...
internal sealed unsafe class DeflaterEngine : IDisposable
{
        ...
        if (DeflaterConstants.COMPR_FUNC[level] != this.compressionFunction)
        {
            switch (this.compressionFunction)
            {
                ...
                case DeflaterConstants.DEFLATE_SLOW:
                    ...
                    {
                        DebugGuard.MustBeGreaterThanOrEqualTo(this.strstart - 1, 0, nameof(this.strstart));
                        DebugGuard.MustBeLessThan(this.strstart - 1, 2 * DeflaterConstants.WSIZE, nameof(this.strstart));
    ...
    public void FillWindow()
    {
        ...
        if (this.lookahead < DeflaterConstants.MIN_LOOKAHEAD && this.inputOff < this.inputEnd)
        {
            ...
            // Validate destination offset and copy length are within bounds of the window buffer.
            DebugGuard.MustBeGreaterThanOrEqualTo(more, 0, nameof(more));
            DebugGuard.MustBeLessThanOrEqualTo(
                this.strstart + this.lookahead + more,
                2 * DeflaterConstants.WSIZE,
    ...
    private int InsertString()
    {
        ...
        // Validate strstart allows access to strstart + MIN_MATCH - 1 within window buffer.
        DebugGuard.MustBeBetweenOrEqualTo(
            this.strstart + (DeflaterConstants.MIN_MATCH - 1),
            0,
    ...
    private bool FindLongestMatch(int curMatch)
    {
        ...
        DebugGuard.MustBeLessThanOrEqualTo(
            scan + DeflaterConstants.MAX_MATCH,
            2 * DeflaterConstants.WSIZE,
            nameof(this.strstart));
    ...
    private bool DeflateFast(bool flush, bool finish)
    {
        ...
        while (this.lookahead >= DeflaterConstants.MIN_LOOKAHEAD || flush)
        {
            ...
            {
                // No match found
                DebugGuard.MustBeBetweenOrEqualTo(this.strstart, 0, (2 * DeflaterConstants.WSIZE) - 1, nameof(this.strstart));
