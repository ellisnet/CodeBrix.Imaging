...
using CodeBrix.Imaging.Fonts.Tables.AdvancedTypographic;
...
/// <summary>
///     Specifies the color font format used by a font, if any.
/// </summary>
public enum ColorFontFormat
{
    /// <summary>
    ///     The font does not contain color glyph data.
    /// </summary>
    None,

    /// <summary>
    ///     The font uses COLRv0 format (simple layered color glyphs with palette indices).
    /// </summary>
    COLRv0,

    /// <summary>
    ///     The font uses COLRv1 format (advanced color glyphs with gradients, transforms, and compositing).
    /// </summary>
    COLRv1
}
...
public abstract class FontMetrics
{
    ...
    /// <summary>
    ///     Gets the suggested distance of the top of the underline from the baseline (negative values indicate below
    ///     baseline).
    /// </summary>
    public abstract short UnderlinePosition { get; }

    /// <summary>
    ///     Gets the suggested values for the underline thickness. In general, the underline thickness should match the
    ///     thickness of
    ///     the underscore character (U+005F LOW LINE), and should also match the strikeout thickness, which is specified in
    ///     the OS/2 table.
    /// </summary>
    public abstract short UnderlineThickness { get; }

    /// <summary>
    ///     Gets the italic angle in counter-clockwise degrees from the vertical. Zero for upright text, negative for text that
    ///     leans to the right (forward).
    /// </summary>
    public abstract float ItalicAngle { get; }

    /// <summary>
    ///     Gets a value indicating whether this font contains color glyph data (COLR/CPAL tables).
    ///     Color fonts can render glyphs with multiple colors defined in the font itself.
    /// </summary>
    public abstract bool IsColorFont { get; }

    /// <summary>
    ///     Gets the color font format used by this font.
    ///     Returns <see cref="ColorFontFormat.None"/> if the font does not contain color glyph data.
    /// </summary>
    public abstract ColorFontFormat ColorFontFormat { get; }
    ...
    ///     if the codepoint is found; otherwise,
    ///     <value>0</value>
    ///     .
    ///     This parameter is passed uninitialized.
    /// </param>
    /// <returns>
    ///     <see langword="true" /> if the face contains a glyph for the specified codepoint; otherwise,
    ///     <see langword="false" />.
    /// </returns>
    ...
    ///     <see langword="false" />.
    /// </returns>
    internal abstract bool TryGetGlyphId(CodePoint codePoint, CodePoint? nextCodePoint, out ushort glyphId,
    ...
    internal abstract bool TryGetMarkAttachmentClass(ushort glyphId,
        out GlyphClassDef markAttachmentClass);
    ...
    ///     <value>0</value>
    ///     the default fallback metrics are returned.
    /// </param>
    /// <param name="support">Options for enabling color font support during layout and rendering.</param>
    /// <returns>The <see cref="IEnumerable{GlyphMetrics}" />.</returns>
    internal abstract IEnumerable<GlyphMetrics> GetGlyphMetrics(CodePoint codePoint, ushort glyphId,
