...
namespace CodeBrix.Imaging.Fonts.Tables.AdvancedTypographic;
...
internal abstract class CoverageTable
{
    ...
    public static CoverageTable Load(BigEndianBinaryReader reader, long offset)
    {
        ...
        var coverageFormat = reader.ReadUInt16();
        return coverageFormat switch
        {
            ...
            _ => throw new InvalidFontFileException(
                $"Invalid value for 'coverageFormat' {coverageFormat}. Should be '1' or '2'.")
        };
    }

    public static CoverageTable[] LoadArray(BigEndianBinaryReader reader, long offset,
        ReadOnlySpan<ushort> coverageOffsets)
    {
        var tables = new CoverageTable[coverageOffsets.Length];
        for (var i = 0; i < tables.Length; i++) tables[i] = Load(reader, offset + coverageOffsets[i]);
...
internal sealed class CoverageFormat1Table : CoverageTable
{
    ...
    {
        var n = Array.BinarySearch(glyphArray, glyphId);
        return n < 0 ? -1 : n;
    }
    ...
    public static CoverageFormat1Table Load(BigEndianBinaryReader reader)
    {
        ...
        var glyphArray = reader.ReadUInt16Array(glyphCount);
...
internal sealed class CoverageFormat2Table : CoverageTable
{
    ...
    public override int CoverageIndexOf(ushort glyphId)
    {
        ...
        {
            var rec = records[i];
            if (rec.StartGlyphId <= glyphId && glyphId <= rec.EndGlyphId) return rec.Index + glyphId - rec.StartGlyphId;
        }
