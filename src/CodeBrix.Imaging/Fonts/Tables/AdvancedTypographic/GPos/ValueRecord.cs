...
namespace CodeBrix.Imaging.Fonts.Tables.AdvancedTypographic.GPos;
...
///     to add to (positive values) or subtract from (negative values) the placement and advance values provided in the
///     font.
...
internal readonly struct ValueRecord
{
    ...
    public ValueRecord(BigEndianBinaryReader reader, ValueFormat valueFormat)
    {
        ...
        // |          |                  | PairPosFormat1 lookup subtable) — may be NULL.                                       |
        // +----------+------------------+--------------------------------------------------------------------------------------+
        XPlacement = (valueFormat & ValueFormat.XPlacement) != 0 ? reader.ReadInt16() : (short)0;
        YPlacement = (valueFormat & ValueFormat.YPlacement) != 0 ? reader.ReadInt16() : (short)0;
        XAdvance = (valueFormat & ValueFormat.XAdvance) != 0 ? reader.ReadInt16() : (short)0;
        YAdvance = (valueFormat & ValueFormat.YAdvance) != 0 ? reader.ReadInt16() : (short)0;
        XPlacementDeviceOffset = (valueFormat & ValueFormat.XPlacementDevice) != 0 ? reader.ReadInt16() : (short)0;
        YPlacementDeviceOffset = (valueFormat & ValueFormat.YPlacementDevice) != 0 ? reader.ReadInt16() : (short)0;
        XAdvanceDeviceOffset = (valueFormat & ValueFormat.XAdvanceDevice) != 0 ? reader.ReadInt16() : (short)0;
        YAdvanceDeviceOffset = (valueFormat & ValueFormat.YAdvanceDevice) != 0 ? reader.ReadInt16() : (short)0;
    }
