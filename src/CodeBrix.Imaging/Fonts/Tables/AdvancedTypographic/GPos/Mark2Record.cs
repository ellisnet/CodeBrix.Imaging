...
/// <summary>
///     A Mark2Record declares one Anchor table for each mark class (including Class 0) identified in the MarkRecords of
///     the MarkArray.
///     Each Anchor table specifies one mark2 attachment point used to attach all the mark1 glyphs in a particular class to
///     the mark2 glyph.
/// </summary>
internal class Mark2Record
{
    ...
    public Mark2Record(BigEndianBinaryReader reader, ushort markClassCount, long offset)
    {
        ...
        // +--------------+------------------------------------+--------------------------------------------------------------------------------------+
        var mark2AnchorOffsets = new ushort[markClassCount];
        MarkAnchorTable = new AnchorTable[markClassCount];
        for (var i = 0; i < markClassCount; i++) mark2AnchorOffsets[i] = reader.ReadOffset16();

        var position = reader.BaseStream.Position;
        for (var i = 0; i < markClassCount; i++)
            if (mark2AnchorOffsets[i] != 0)
                MarkAnchorTable[i] = AnchorTable.Load(reader, offset + mark2AnchorOffsets[i]);
