...
///     Each EntryExitRecord consists of two offsets: one to an Anchor table that identifies the entry point on the glyph
///     (entryAnchorOffset),
...
internal readonly struct EntryExitRecord
{
    ...
    public EntryExitRecord(BigEndianBinaryReader reader, long offset)
    {
        ...
        // |              |                        | CursivePos subtable (may be NULL).             |
        // +--------------+------------------------+------------------------------------------------+
        EntryAnchorOffset = reader.ReadOffset16();
        ExitAnchorOffset = reader.ReadOffset16();
    }
