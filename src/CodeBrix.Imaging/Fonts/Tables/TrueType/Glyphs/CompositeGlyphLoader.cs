...
namespace CodeBrix.Imaging.Fonts.Tables.TrueType.Glyphs;
...
internal sealed class CompositeGlyphLoader : GlyphLoader
{
    ...
    public override GlyphVector CreateGlyph(GlyphTable table)
    {
        ...
        {
            ref var composite = ref result[resultIndex];
            glyph = GlyphVector.Append(glyph,
                GlyphVector.Transform(GlyphVector.DeepClone(table.GetGlyph(composite.GlyphIndex)),
                    composite.Transformation), bounds);
        }
    ...
    public static CompositeGlyphLoader LoadCompositeGlyph(BigEndianBinaryReader reader, in Bounds bounds)
    {
        ...
        do
        {
            ...
            LoadArguments(reader, flags, out var dx, out var dy);
            ...
            {
                var scale = reader.ReadF2dot14(); // Format 2.14
                transform.M11 = scale;
                transform.M21 = scale;
            }
