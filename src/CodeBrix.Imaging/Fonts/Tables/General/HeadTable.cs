...
internal class HeadTable : Table
{
        ...
        IndexLocationFormats indexToLocFormat)
    {
        ...
        IndexLocationFormat = indexToLocFormat;
    }
    ...
    public static HeadTable Load(BigEndianBinaryReader reader)
    {
        ...
        if (magicNumber != 0x5F0F3CF5) throw new InvalidFontFileException("invalid magic number in 'head'");

        var flags = reader.ReadUInt16<HeadFlags>();
        var unitsPerEm = reader.ReadUInt16();
        if (unitsPerEm < 16 || unitsPerEm > 16384)
            throw new InvalidFontFileException(
                $"invalid units per em expected value between 16 and 16384 but found {unitsPerEm} in 'head'");
