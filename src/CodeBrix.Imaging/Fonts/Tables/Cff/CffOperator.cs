...
internal sealed class CFFOperator
{
    ...
    {
        RegisteredOperators.Value.TryGetValue((b1 << 8) | b0, out var found);
        return found!;
    }
    ...
    private static Dictionary<int, CFFOperator> CreateDictionary()
    {
        ...
        Register(dictionary, 12, 17, "LanguageGroup",
            OperatorOperandKind.Number); // https://typekit.files.wordpress.com/2013/05/5176.cff.pdf
        Register(dictionary, 12, 18, "ExpansionFactor",
            OperatorOperandKind.Number); // https://typekit.files.wordpress.com/2013/05/5176.cff.pdf
        Register(dictionary, 12, 19, "initialRandomSeed",
            OperatorOperandKind.Number); // https://typekit.files.wordpress.com/2013/05/5176.cff.pdf
    ...
    private static void Register(Dictionary<int, CFFOperator> dictionary, byte b0, byte b1, string operatorName,
        OperatorOperandKind opopKind)
    {
        dictionary.Add((b1 << 8) | b0, new CFFOperator(operatorName, b0, b1, opopKind));
    }

    private static void Register(Dictionary<int, CFFOperator> dictionary, byte b0, string operatorName,
        OperatorOperandKind opopKind)
    {
