...
namespace CodeBrix.Imaging.Fonts.Unicode;
...
internal class UnicodeTrieBuilder
{
    ...
    public UnicodeTrieBuilder(uint initialValue = 0, uint errorValue = 0)
    {
        ...
        index1 = new int[INDEX_1_LENGTH];
        index2 = new int[UNEWTRIE2_MAX_INDEX_2_LENGTH];
        data = new uint[INITIAL_DATA_LENGTH];
        ...
        map = new int[UNEWTRIE2_MAX_DATA_LENGTH >> UTRIE2_SHIFT_2];
        ...
        // - the null data block
        int i;
        for (i = 0; i < 0x80; ++i) data[i] = initialValue;

        for (; i < 0xc0; ++i) data[i] = errorValue;

        for (i = UNEWTRIE2_DATA_NULL_OFFSET; i < UNEWTRIE2_DATA_START_OFFSET; ++i) data[i] = initialValue;
        ...
        for (; j < 0xc0; ++i, j += UTRIE2_DATA_BLOCK_LENGTH) map[i] = 0;

        // Reference counts for the null data block: all blocks except for the ASCII blocks.
        // Plus 1 so that we don't drop this block during compaction.
        // Plus as many as needed for lead surrogate code points.
        // i==newdataNullOffset
        map[i++] = (0x110000 >> UTRIE2_SHIFT_2)
                   - (0x80 >> UTRIE2_SHIFT_2)
                   + 1
                   + UTRIE2_LSCP_INDEX_2_LENGTH;

        j += UTRIE2_DATA_BLOCK_LENGTH;
        for (; j < UNEWTRIE2_DATA_START_OFFSET; ++i, j += UTRIE2_DATA_BLOCK_LENGTH) map[i] = 0;

        // set the remaining indexes in the BMP index-2 block
        // to the null data block
        for (i = 0x80 >> UTRIE2_SHIFT_2; i < UTRIE2_INDEX_2_BMP_LENGTH; ++i) index2[i] = UNEWTRIE2_DATA_NULL_OFFSET;

        // Fill the index gap with impossible values so that compaction
        // does not overlap other index-2 blocks with the gap.
        for (i = 0; i < UNEWTRIE2_INDEX_GAP_LENGTH; ++i) index2[UNEWTRIE2_INDEX_GAP_OFFSET + i] = -1;
        ...
        for (i = 0, j = 0; i < UTRIE2_OMITTED_BMP_INDEX_1_LENGTH; ++i, j += UTRIE2_INDEX_2_BLOCK_LENGTH) index1[i] = j;

        // set the remaining index-1 indexes to the null index-2 block
        for (; i < UNEWTRIE2_INDEX_1_LENGTH; ++i) index1[i] = UNEWTRIE2_INDEX_2_NULL_OFFSET;

        // Preallocate and reset data for U+0080..U+07ff,
        // for 2-byte UTF-8 which will be compacted in 64-blocks
        // even if UTRIE2_DATA_BLOCK_LENGTH is smaller.
        for (i = 0x80; i < 0x800; i += UTRIE2_DATA_BLOCK_LENGTH) Set(i, initialValue);
    }
    ...
    {
        if (codePoint is < 0 or > 0x10ffff) throw new ArgumentOutOfRangeException("Invalid code point");

        if (isCompacted) throw new InvalidOperationException("Already compacted");

        var block = GetDataBlock(codePoint, true);
        data[block + (codePoint & UTRIE2_DATA_MASK)] = value;
    }
    ...
    public void SetRange(int start, int end, uint value, bool overwrite)
    {
        ...
        if (!overwrite && value == initialValue) return; // Nothing to do.
        ...
        if ((start & UTRIE2_DATA_MASK) != 0)
        {
            ...
            if (block < 0) throw new IndexOutOfRangeException(nameof(block));

            nextStart = (start + UTRIE2_DATA_MASK) & ~UTRIE2_DATA_MASK;
            if (nextStart <= limit)
            {
                FillBlock(block, start & UTRIE2_DATA_MASK, UTRIE2_DATA_BLOCK_LENGTH, value, initialValue, overwrite);
                start = nextStart;
            }
            else
            {
                FillBlock(block, start & UTRIE2_DATA_MASK, limit & UTRIE2_DATA_MASK, value, initialValue, overwrite);
                return;
            }
        }
        ...
        while (start < limit)
        {
            ...
            if (i2 < 0) throw new IndexOutOfRangeException(nameof(i2));
            ...
            if (setRepeatBlock)
            {
                ...
                else
                {
                    ...
                    if (repeatBlock < 0) throw new IndexOutOfRangeException(nameof(repeatBlock));
    ...
    public UnicodeTrie Freeze()
    {
        ...
        if (allIndexesLength > UTRIE2_MAX_INDEX_LENGTH // for unshifted indexLength
            || dataMove + dataNullOffset > 0xffff // for unshifted dataNullOffset
        ...
        var indexLength = allIndexesLength + dataLength;
        var data32 = new uint[indexLength];
        ...
        for (i = 0; i < UTRIE2_INDEX_2_BMP_LENGTH; i++)
            data32[destIdx++] = (uint)((index2[i] + dataMove) >> UTRIE2_INDEX_SHIFT);
        ...
        for (; i < 0xe0 - 0xc0; i++)
            // C2..DF
            data32[destIdx++] = (uint)(dataMove + index2[i << (6 - UTRIE2_SHIFT_2)]);

        if (highStart > 0x10000)
        {
            var index1Length = (highStart - 0x10000) >> UTRIE2_SHIFT_1;
            var index2Offset = UTRIE2_INDEX_2_BMP_LENGTH + UTRIE2_UTF8_2B_INDEX_2_LENGTH + index1Length;

            // write 16-bit index-1 values for supplementary code points
            for (i = 0; i < index1Length; i++)
                data32[destIdx++] = (uint)(UTRIE2_INDEX_2_OFFSET + index1[i + UTRIE2_OMITTED_BMP_INDEX_1_LENGTH]);

            // write the index-2 array values for supplementary code points,
            // shifted right by INDEX_SHIFT, after adding dataMove
            for (i = 0; i < index2Length - index2Offset; i++)
                data32[destIdx++] = (uint)((dataMove + index2[index2Offset + i]) >> UTRIE2_INDEX_SHIFT);
        }

        // write 16-bit data values
        for (i = 0; i < dataLength; i++) data32[destIdx++] = data[i];

        return new UnicodeTrie(data32, highStart, errorValue);
    }

    private uint Get(int c, bool fromLSCP)
    {
        if (c is < 0 or > 0x10ffff) return errorValue;

        int i2;
        int block;

        if (c >= highStart && (!U_IS_LEAD(c) || fromLSCP)) return data[dataLength - UTRIE2_DATA_GRANULARITY];
    ...
    private int GetDataBlock(int c, bool forLSCP)
    {
        ...
        var oldBlock = index2[i2];
        if (IsWritableBlock(oldBlock)) return oldBlock;

        // allocate a new data block
        var newBlock = AllocDataBlock(oldBlock);

        if (newBlock < 0) throw new IndexOutOfRangeException(nameof(newBlock));
    ...
    {
        if (U_IS_LEAD(c) && forLSCP) return UTRIE2_LSCP_INDEX_2_OFFSET;
    ...
    {
        return block != dataNullOffset && map[block >> UTRIE2_SHIFT_2] == 1;
    }
    ...
    private void SetIndex2Entry(int i2, int block)
    {
        ...
        ++map[block >> UTRIE2_SHIFT_2];

        oldBlock = index2[i2];

        if (--map[oldBlock >> UTRIE2_SHIFT_2] == 0) ReleaseDataBlock(oldBlock);
    ...
    private int AllocDataBlock(int copyBlock)
    {
        ...
        Array.Copy(data, copyBlock, data, newBlock, UTRIE2_DATA_BLOCK_LENGTH);
    ...
    private int AllocIndex2Block()
    {
        ...
        index2Length = newTop;
        Array.Copy(index2, index2NullOffset, index2, newBlock, UTRIE2_INDEX_2_BLOCK_LENGTH);
    ...
    private int FindSameIndex2Block(int index2Length, int otherBlock)
    {
        ...
        for (var block = 0; block <= index2Length; ++block)
            if (Equal(index2, block, otherBlock, UTRIE2_INDEX_2_BLOCK_LENGTH))
                return block;
    ...
    private int FindSameDataBlock(int dataLength, int otherBlock, int blockLength)
    {
        ...
        for (var block = 0; block <= dataLength; block += UTRIE2_DATA_GRANULARITY)
            if (Equal(data, block, otherBlock, blockLength))
                return block;
    ...
    private void FillBlock(int block, int start, int limit, uint value, uint initialValue, bool overwrite)
    {
        ...
        if (overwrite)
            ...
            while (block < pLimit)
            {
                if (data[block] == initialValue) data[block] = value;
    ...
    private void WriteBlock(int block, uint value)
    {
        ...
        while (block < limit) data[block++] = value;
    }
    ...
    private void CompactTrie()
    {
        ...
        var localHighStart = FindHighStart(highValue);
        localHighStart = (localHighStart + (UTRIE2_CP_PER_INDEX_1_ENTRY - 1)) & ~(UTRIE2_CP_PER_INDEX_1_ENTRY - 1);
        if (localHighStart == 0x110000) highValue = errorValue;
        ...
        CompactData();
        if (highStart > 0x10000) CompactIndex2();

        // Store the highValue in the data array and round up the dataLength.
        // Must be done after compactData() because that assumes that dataLength
        // is a multiple of UTRIE2_DATA_BLOCK_LENGTH.
        data[dataLength++] = highValue;
        while ((dataLength & (UTRIE2_DATA_GRANULARITY - 1)) != 0) data[dataLength++] = initialValue;
    ...
    private void CompactData()
    {
        ...
        for (start = 0, i = 0; start < newStart; start += UTRIE2_DATA_BLOCK_LENGTH, ++i) map[i] = start;
        ...
        for (start = newStart; start < dataLength;)
        {
            ...
            while (overlap > 0 && !Equal(data, newStart - overlap, start, overlap)) overlap -= UTRIE2_DATA_GRANULARITY;
            ...
            if (overlap > 0 || newStart < start)
            {
                ...
                start += overlap;
                for (i = blockLength - overlap; i > 0; --i) data[newStart++] = data[start++];
            }
            else
            {
        ...
        while ((newStart & (UTRIE2_DATA_GRANULARITY - 1)) != 0) data[newStart++] = initialValue;
    ...
    private void CompactIndex2()
    {
        ...
        for (start = 0, i = 0; start < newStart; start += UTRIE2_INDEX_2_BLOCK_LENGTH, ++i) map[i] = start;
        ...
        for (start = UNEWTRIE2_INDEX_2_NULL_OFFSET; start < index2Length;)
        {
            ...
            for (overlap = UTRIE2_INDEX_2_BLOCK_LENGTH - 1;
                 overlap > 0 && !Equal(index2, newStart - overlap, start, overlap);
                 --overlap) { }
            ...
            if (overlap > 0 || newStart < start)
            {
                ...
                // move the non-overlapping indexes to their new positions
                start += overlap;
                for (i = UTRIE2_INDEX_2_BLOCK_LENGTH - overlap; i > 0; --i) index2[newStart++] = index2[start++];
            }
            else
            {
                // no overlap && newStart==start
        ...
        for (i = 0; i < UNEWTRIE2_INDEX_1_LENGTH; ++i) index1[i] = map[index1[i] >> UTRIE2_SHIFT_1_2];
        ...
        while ((newStart & ((UTRIE2_DATA_GRANULARITY - 1) | 1)) != 0)
            ...
            index2[newStart++] = 0xffff << UTRIE2_INDEX_SHIFT;
