...
public ref struct SpanGraphemeEnumerator
{
    ...
    public bool MoveNext()
    {
        ...
        while (processor.CurrentType == GraphemeClusterClass.Prepend) processor.MoveNext();
        ...
        switch (previousClusterBreakType)
        {
            case GraphemeClusterClass.CarriageReturn:
                if (processor.CurrentType !=
                    GraphemeClusterClass.LineFeed) goto Return; // rules GB3 & GB4 (only <LF> can follow <CR>)
            ...
            case GraphemeClusterClass.HangulLead:
                ...
                if (processor.CurrentType == GraphemeClusterClass.HangulLeadVowel)
                {
                    processor.MoveNext(); // rule GB6 (L x LV)
                    goto case GraphemeClusterClass.HangulLeadVowel;
                }

                if (processor.CurrentType == GraphemeClusterClass.HangulLeadVowelTail)
                {
            ...
            case GraphemeClusterClass.ExtendedPictographic:
                ...
                while (processor.CurrentType == GraphemeClusterClass.Extend) processor.MoveNext();

                // Now see if there's a ZWJ + extended pictograph again.
                if (processor.CurrentType != GraphemeClusterClass.ZeroWidthJoiner) break;

                processor.MoveNext();
                if (processor.CurrentType != GraphemeClusterClass.ExtendedPictographic) break;
            ...
            case GraphemeClusterClass.RegionalIndicator:
                ...
                if (processor.CurrentType == GraphemeClusterClass.RegionalIndicator) processor.MoveNext();
        ...
        Current = source.Slice(0, processor.CharsConsumed);
        source = source.Slice(processor.CharsConsumed);
    ...
    private ref struct Processor
    {
        ...
        {
            this.source = source;
            CurrentType = GraphemeClusterClass.Any;
            charsConsumed = 0;
            CharsConsumed = 0;
        }
        ...
        public void MoveNext()
        {
            ...
            var codePoint = CodePoint.DecodeFromUtf16At(source, CharsConsumed, out charsConsumed);
            CurrentType = CodePoint.GetGraphemeClusterClass(codePoint);
        }
    }
}