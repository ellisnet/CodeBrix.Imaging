...
internal static class UnicodeUtility
{
    ...
    ///     <br />
    ///     <see
    ///         href="https://github.com/harfbuzz/harfbuzz/blob/a52c6df38a38c4e36ff991dfb4b7d92e48a44553/src/hb-ot-shape.cc#L652-L701" />
    /// </summary>
    /// <returns>
    ...
    {
        // Hiragana
        if (IsInRangeInclusive(value, 0x3040u, 0x309Fu)) return true;

        // Katakana
        if (IsInRangeInclusive(value, 0x30A0u, 0x30FFu)) return true;

        // Hangul Syllables
        if (IsInRangeInclusive(value, 0xAC00u, 0xD7A3u)) return true;

        // CJK Unified Ideographs
        if (IsInRangeInclusive(value, 0x4E00u, 0x9FFFu)) return true;

        // CJK Unified Ideographs Extension A
        if (IsInRangeInclusive(value, 0x3400u, 0x4DBFu)) return true;

        // CJK Unified Ideographs Extension B
        if (IsInRangeInclusive(value, 0x20000u, 0x2A6DFu)) return true;

        // CJK Unified Ideographs Extension C
        if (IsInRangeInclusive(value, 0x2A700u, 0x2B73Fu)) return true;

        // CJK Unified Ideographs Extension D
        if (IsInRangeInclusive(value, 0x2B740u, 0x2B81Fu)) return true;

        // CJK Unified Ideographs Extension E
        if (IsInRangeInclusive(value, 0x2B820u, 0x2CEAFu)) return true;

        // CJK Unified Ideographs Extension F
        if (IsInRangeInclusive(value, 0x2CEB0u, 0x2EBEFu)) return true;

        // CJK Unified Ideographs Extension G
        if (IsInRangeInclusive(value, 0x30000u, 0x3134Fu)) return true;

        // CJK Compatibility Ideographs
        if (IsInRangeInclusive(value, 0xF900u, 0xFAFFu)) return true;

        // CJK Compatibility Ideographs Supplement
        if (IsInRangeInclusive(value, 0x2F800u, 0x2FA1Fu)) return true;
    ...
    public static bool IsDefaultIgnorableCodePoint(uint value)
    {
        ...
        if (IsInRangeInclusive(value, 0x115F, 0x1160)) return true;

        // KHMER VOWEL INHERENT AQ..KHMER VOWEL INHERENT AA
        if (IsInRangeInclusive(value, 0x17B4, 0x17B5)) return true;

        // MONGOLIAN FREE VARIATION SELECTOR ONE..MONGOLIAN FREE VARIATION SELECTOR THREE
        if (IsInRangeInclusive(value, 0x180B, 0x180D)) return true;
        ...
        if (IsInRangeInclusive(value, 0x200B, 0x200F)) return true;

        // LEFT-TO-RIGHT EMBEDDING..RIGHT-TO-LEFT OVERRIDE
        if (IsInRangeInclusive(value, 0x202A, 0x202E)) return true;

        // WORD JOINER..INVISIBLE PLUS
        if (IsInRangeInclusive(value, 0x2060, 0x2064)) return true;

        // <reserved-2065>
        if (value == 0x2065) return true;

        // LEFT-TO-RIGHT ISOLATE..NOMINAL DIGIT SHAPES
        if (IsInRangeInclusive(value, 0x2066, 0x206F)) return true;

        // HANGUL FILLER
        if (value == 0x3164) return true;

        // VARIATION SELECTOR-1..VARIATION SELECTOR-16
        if (IsInRangeInclusive(value, 0xFE00, 0xFE0F)) return true;
        ...
        if (IsInRangeInclusive(value, 0xFFF0, 0xFFF8)) return true;

        // SHORTHAND FORMAT LETTER OVERLAP..SHORTHAND FORMAT UP STEP
        if (IsInRangeInclusive(value, 0x1BCA0, 0x1BCA3)) return true;

        // MUSICAL SYMBOL BEGIN BEAM..MUSICAL SYMBOL END PHRASE
        if (IsInRangeInclusive(value, 0x1D173, 0x1D17A)) return true;
        ...
        if (IsInRangeInclusive(value, 0xE0002, 0xE001F)) return true;

        // TAG SPACE..CANCEL TAG
        if (IsInRangeInclusive(value, 0xE0020, 0xE007F)) return true;

        // <reserved-E0080>..<reserved-E00FF>
        if (IsInRangeInclusive(value, 0xE0080, 0xE00FF)) return true;

        // VARIATION SELECTOR-17..VARIATION SELECTOR-256
        if (IsInRangeInclusive(value, 0xE0100, 0xE01EF)) return true;

        // <reserved-E01F0>..<reserved-E0FFF>
        if (IsInRangeInclusive(value, 0xE01F0, 0xE0FFF)) return true;
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void GetUtf16SurrogatesFromSupplementaryPlaneCodePoint(uint value, out char highSurrogateCodePoint,
    ...
    internal static void DebugAssertIsValidSupplementaryPlaneCodePoint(uint codePoint)
    {
        if (!IsValidCodePoint(codePoint) || IsBmpCodePoint(codePoint))
            Debug.Fail(
                $"The value {ToHexString(codePoint)} is not a valid supplementary plane Unicode code point value.");
    }
    ...
    /// </remarks>
    internal static string ToHexString(uint codePoint)
    {
