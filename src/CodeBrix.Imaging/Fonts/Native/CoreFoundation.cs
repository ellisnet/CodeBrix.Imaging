...
namespace CodeBrix.Imaging.Fonts.Native;
...
internal static class CoreFoundation
{
    ...
    ///     <paramref name="theArray" /> (<c>0</c> to <c>N-1</c> inclusive where <c>N</c> is the count of
    ///     <paramref name="theArray" />), the behavior is undefined.
    /// </param>
    /// <returns>
    ///     The value at the <paramref name="idx" /> index in <paramref name="theArray" />. If the return value is a Core
    ...
    ///     You can compare this value with the known <c>CFTypeID</c> identifier obtained with a “GetTypeID” function specific
    ///     to a type, for example CFDateGetTypeID.
    ...
    /// <summary>
    ///     Copies the character contents of a string to a local C string buffer after converting the characters to a given
    ///     encoding.
    /// </summary>
    /// <param name="theString">The string whose contents you wish to access.</param>
    /// <param name="buffer">
    ///     The C string buffer into which to copy the string. On return, the buffer contains the converted characters. If
    ///     there is an error in conversion, the buffer contains only partial results.
    ///     The buffer must be large enough to contain the converted characters and a NUL terminator. For example, if the
    ///     string is <c>Toby</c>, the buffer must be at least 5 bytes long.
    /// </param>
    /// <param name="bufferSize">The length of <paramref name="buffer" /> in bytes.</param>
    /// <param name="encoding">
    ///     The string encoding to which the character contents of <paramref name="theString" /> should be
    ///     converted. The encoding must specify an 8-bit encoding.
    /// </param>
    /// <returns>
    ///     <see langword="true" /> upon success or <see langword="false" /> if the conversion fails or the provided
    ///     buffer is too small.
    /// </returns>
    /// <remarks>
    ///     This function is useful when you need your own copy of a string’s character data as a C string. You also
    ///     typically call it as a “backup” when a prior call to the <see cref="CFStringGetCStringPtr" /> function fails.
    /// </remarks>
    [DllImport(CoreFoundationFramework, CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl,
        ExactSpelling = true)]
    public static extern bool CFStringGetCString(IntPtr theString, byte[] buffer, long bufferSize,
    ...
    /// </param>
    /// <returns>
    ///     A pointer to a C string or <c>NULL</c> if the internal storage of <paramref name="theString" /> does not allow
    ///     this to be returned efficiently.
    /// </returns>
    /// <remarks>
    ///     <para>
    ///         This function either returns the requested pointer immediately, with no memory allocations and no copying, in
    ///         constant time, or returns <c>NULL</c>. If the latter is the result, call an alternative function such as the
    ///         <see cref="CFStringGetCString" /> function to extract the characters.
    ///     </para>
    ///     <para>
    ///         Whether or not this function returns a valid pointer or <c>NULL</c> depends on many factors, all of which
    ///         depend on how the string was created and its properties. In addition, the function result might change between
    ///         different releases and on different platforms. So do not count on receiving a non-<c>NULL</c> result from this
    ...
    /// <param name="pathStyle">
    ///     The operating system path style to be used to create the path. See
    ///     <see cref="CFURLPathStyle" /> for a list of possible values.
    /// </param>
    /// <returns>
    ///     The URL's path in the format specified by <paramref name="pathStyle" />. Ownership follows the create rule.
    ///     See The Create Rule.
    /// </returns>
