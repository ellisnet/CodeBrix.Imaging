...
internal class BigEndianBinaryReader : IDisposable
{
    ...
    /// <param name="origin">
    ///     Origin of seek operation. If SeekOrigin.Begin, the offset will be set to the start of stream
    ///     position.
    /// </param>
    public void Seek(long offset, SeekOrigin origin)
    {
        // If SeekOrigin.Begin, the offset will be set to the start of stream position.
        if (origin == SeekOrigin.Begin) offset += StartOfStream;
    ...
    public float ReadFixed()
    {
        ...
        return BinaryPrimitives.ReadInt32BigEndian(buffer) / 65536F;
    }
    ...
    public ushort[] ReadUInt16Array(int length)
    {
        ...
        for (var i = 0; i < length; i++) data[i] = ReadUInt16();
    ...
    {
        for (var i = 0; i < buffer.Length; i++) buffer[i] = ReadUInt16();
    }
    ...
    public uint[] ReadUInt32Array(int length)
    {
        ...
        for (var i = 0; i < length; i++) data[i] = ReadUInt32();
    ...
    public short[] ReadInt16Array(int length)
    {
        ...
        for (var i = 0; i < length; i++) data[i] = ReadInt16();
    ...
    {
        for (var i = 0; i < buffer.Length; i++) buffer[i] = ReadInt16();
    }
    ...
    ///     3 bytes are read.
    /// </summary>
    /// <returns>The 24-bit offset value as a 32-bit unsigned integer.</returns>
    public uint ReadOffset24()
    {
        return (uint)ReadUInt24();
    }

    /// <summary>
    ///     Reads a F2DOT14 fixed-point value from the stream.
    ///     2 bytes are read.
    /// </summary>
    ...
    /// <summary>
    ///     Reads a signed 16-bit integer representing a value in font units (FWord).
    ///     2 bytes are read.
    /// </summary>
    ...
    /// <summary>
    ///     Reads an unsigned 16-bit integer representing a value in font units (UFWord).
    ///     2 bytes are read.
    /// </summary>
