...
using System.IO;
using CodeBrix.Imaging.Fonts.Tables;
using CodeBrix.Imaging.Fonts.Tables.AdvancedTypographic;
using CodeBrix.Imaging.Fonts.Tables.Cff;
using CodeBrix.Imaging.Fonts.Tables.General;
using CodeBrix.Imaging.Fonts.Tables.General.Colr;
using CodeBrix.Imaging.Fonts.Tables.TrueType;
...
internal partial class StreamFontMetrics : FontMetrics
{
    ...
    /// <inheritdoc />
    public override bool IsColorFont => colorGlyphCache is not null;

    /// <inheritdoc />
    public override ColorFontFormat ColorFontFormat
    {
        get
        {
            if (colorGlyphCache is null)
            {
            ...
            return colr.HasV1Data ? ColorFontFormat.COLRv1 : ColorFontFormat.COLRv0;
        }
    }
    ...
    /// <inheritdoc />
    internal override bool TryGetGlyphClass(ushort glyphId, out GlyphClassDef glyphClass)
    {
        ...
        [NotNullWhen(true)] out GlyphMetrics[] metrics)
    {
        ...
        if (metrics is null)
        {
            // Try COLRv1 first if available
            var cpal = outlineType == OutlineType.TrueType
                ? trueTypeFontTables!.Cpal
                : compactFontTables!.Cpal;
            
            if (colr.HasV1Data && cpal != null)
            {
                // For COLRv1, we use the foreground color index (0xFFFF means use foreground)
                var foregroundColor = new GlyphColor(0, 0, 0, 255); // Default black foreground
                if (colr.TryGetV1Layers(glyphId, cpal, foregroundColor, out var resolvedLayers))
                {
                    metrics = new GlyphMetrics[resolvedLayers.Count];
                    for (var i = 0; i < resolvedLayers.Count; i++)
                    {
                        var layer = resolvedLayers[i];
                        metrics[i] = CreateGlyphMetricsWithColor(codePoint, layer.GlyphId, GlyphType.ColrLayer, layer.Color);
                    }
                }
            }

            // Fall back to COLRv0 if v1 didn't produce results
            if (metrics is null)
            {
    ...
    private GlyphMetrics CreateGlyphMetricsWithColor(
        CodePoint codePoint,
        ushort glyphId,
        GlyphType glyphType,
        GlyphColor color)
    {
        return outlineType switch
        {
            OutlineType.TrueType => CreateTrueTypeGlyphMetricsWithColor(codePoint, glyphId, glyphType, color),
            OutlineType.CFF => CreateCffGlyphMetricsWithColor(codePoint, glyphId, glyphType, color),
            _ => throw new NotSupportedException()
        };
    }
}