...
public abstract partial class Image : IImage, IConfigurationProvider
{
    ...
    /// <inheritdoc />
    public IImageFormat Format => Metadata.ExpectedFormat;
    ...
    public void Save(Stream stream, IImageEncoder encoder)
    {
        ...
        this.UpdateExpectedFormat(encoder);
    }
    ...
    public async Task SaveAsync(Stream stream, IImageEncoder encoder, CancellationToken cancellationToken = default)
    {
        Guard.NotNull(stream, nameof(stream));
        Guard.NotNull(encoder, nameof(encoder));
        this.EnsureNotDisposed();

        await this.AcceptVisitorAsync(new EncodeVisitor(encoder, stream), cancellationToken).ConfigureAwait(false);
        this.UpdateExpectedFormat(encoder);
    }
    ...
    /// <summary>
    /// Updates the <see cref="ImageMetadata.ExpectedFormat"/> to match the format
    /// associated with the encoder that was used to save the image.
    /// </summary>
